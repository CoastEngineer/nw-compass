...
export default function SnapshotsPage() {
  ...
  const compare = useMemo(() => {
    ...
    // compare milestones (Base) row-by-row by target
    ...
    >
      <div className="flex items-center justify-between gap-3">
        <div className="text-sm text-neutral-600">
          Saved: <span className="font-medium text-neutral-900">{snapshots.length}</span>
        </div>
        <button
        ...
        <div className="mt-6">
          ...
          >
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <MiniKPI label="End NW (Base) — A" value={formatCompactUsd(compare.aBase)} />
              <MiniKPI label="End NW (Base) — B" value={formatCompactUsd(compare.bBase)} />
              <MiniKPI
                label="Delta (B - A)"
